...
export function setTheme(storymap: StoryMapJSON, themeId: string): void {
  ...
  for (const [ resource ] of Object.entries(storymap.resources)) {
    const r = resource as unknown as { type?: string; data?: Record<string, unknown> };
    if (r.type === 'story-theme') {
      if (STANDARD_THEMES.includes(themeId)) {
        if (r.data) {
          r.data.themeId = themeId as unknown as any;
          delete (r.data as any).themeItemId;
        }
      } else {
        if (r.data) {
          r.data.themeItemId = themeId as unknown as any;
          delete (r.data as any).themeId;
        }
      }
      break;
    }
