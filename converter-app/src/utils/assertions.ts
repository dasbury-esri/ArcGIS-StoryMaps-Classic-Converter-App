...
export function assertStoryMapJson(json: StoryMapJSON): AssertionResult {
  ...
  for (const [id, node] of Object.entries(json.nodes)) {
    ...
    if (node.type === 'webmap') {
      const cfg = (isObj((node as any).config) ? ((node as any).config as Record<string, unknown>) : undefined);
      const size = (cfg ? cfg['size'] : undefined) as string | undefined;
      ...
      if (videoRes && !resourceIds.has(videoRes)) warnings.push(`Video node '${id}' references missing resource '${videoRes}'`);
