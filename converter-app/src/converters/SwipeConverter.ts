...
export class SwipeConverter extends BaseConverter {
  ...
  protected convertContent(): void {
    ...
    if (this.model === 'TWO_WEBMAPS') {
      ...
      // Persist enrichment at top-level resource data to match expected schema
      if (resA && infoA) this.builder.updateWebMapData(resA, {
        extent: infoA.extent,
        center: infoA.center,
        viewpoint: (initialA as any).viewpoint,
        mapLayers: (initialA as any).mapLayers,
      });
      if (resB && infoB) this.builder.updateWebMapData(resB, {
        extent: infoB.extent,
        center: infoB.center,
  ...
  static buildInlineSwipeBlock(
    ...
    if (dm === 'TWO_WEBMAPS') {
      ...
      if (contentA && infoA?.extent) builder.updateNodeData(contentA, (data) => {
        (data as Record<string, unknown>).extent = infoA.extent;
        const sz = determineScaleZoomLevel(infoA.extent as unknown as { ymax: number; ymin: number });
        if (sz) (data as Record<string, unknown>).viewpoint = { targetGeometry: infoA.center ?? infoA.extent, scale: sz.scale };
        const hasTime = Array.isArray(infoA.operationalLayers) && infoA.operationalLayers.some(l => (l as any).timeAnimation === true);
